...
RUN_DIR = LAB_DIR / "outputs" / "trot_finetune_5m"
...
def evaluate_gait_symmetry(
    ...
    return rows, float(np.mean(contact_match[25:]))
...
# v2.2（动作滤波，无 trot 条件化）演示基线，
# 数据来自 ../6.rl_pupper_v2_2/reports/raw/command_demo.csv。
V22_DEMO_STATS = {
    (0.0, 0.0, 0.0): {"vx_mean": -0.002, "vx_std": 0.051, "wz_mean": -0.022, "wz_std": 0.385, "z_std": 0.0031},
    (0.5, 0.0, 0.0): {"vx_mean": 0.479, "vx_std": 0.049, "wz_mean": 0.033, "wz_std": 0.391, "z_std": 0.0034},
    (0.0, 0.0, 0.5): {"vx_mean": -0.001, "vx_std": 0.048, "wz_mean": 0.512, "wz_std": 0.367, "z_std": 0.0027},
    (-0.3, 0.0, 0.0): {"vx_mean": -0.363, "vx_std": 0.038, "wz_mean": -0.042, "wz_std": 0.276, "z_std": 0.0062},
}
V22_DEMO_TERMINATIONS = 0
...
def write_report(
    ...
    selected_steps = (1_000_000, 2_000_000, 3_000_000, 4_000_000, 5_000_000)
    by_pair = {
    ...
    comparison_lines.append(
        ...
        f"{V22_DEMO_TERMINATIONS} | {len(demo_terminations)} |"
    )

    symmetry_lines = []
    for (label, v2_duty, v2_lifts), (_, v22_duty, v22_lifts), (_, duty, lifts) in zip(
        V2_GAIT_STATS, V22_GAIT_STATS, symmetry_rows,
    ):
        symmetry_lines.append(
            f"| {label} | {v2_duty:.1f}% / {v2_lifts} | "
            f"{v22_duty:.1f}% / {v22_lifts} | {duty:.1f}% / {lifts} |"
        )
    termination_text = (
        ", ".join(f"t={float(row['time_s']):.2f}s" for row in demo_terminations)
        if demo_terminations else "无"
    )
    report = f"""# Pupper PPO 本机训练报告（v2.3）
...
在 Apple M3 Ultra 上使用 16 个 CPU MuJoCo 环境，在 v2.2 两阶段产物之上完成第三阶段 trot 条件化微调 {final_step:,} 步（耗时约 {duration_minutes:.1f} 分钟）。
...
- 阶段三 trot 条件化：{config['run']['total_timesteps']:,} 步，48 → 53 维迁移 + 接触时序奖励 4.0（配置见 [`raw/finetune_gait.yaml`](raw/finetune_gait.yaml)）
...
**{contact_match * 100:.1f}%**（随机步态基线约 50%）。
...
def main() -> None:
    ...
    symmetry_rows, contact_match = evaluate_gait_symmetry(best_checkpoint)
    write_report(
        machine, config, scalars, summary_rows, aggregate, best_steps,
        symmetry_rows, contact_match,
    )
