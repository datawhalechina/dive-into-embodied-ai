...
- 奖励：速度跟踪、姿态、能耗、平滑性、足端腾空时间、钉腿超时惩罚
  （feet_stance_time，防三腿跛行）、防抖动与防打滑惩罚、站立静止约束、
  拒绝执行命令惩罚（dont_wait）、可选步态接触和终止惩罚。
...
class PupperEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward, info = self._compute_reward(
            action, rotation, contact, slip_contact, first_contact, terminated,
        )
