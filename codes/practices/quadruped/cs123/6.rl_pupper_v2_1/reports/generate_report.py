...
# v2（无钉腿惩罚）演示基线，数据来自 ../6.rl_pupper_v2/reports/raw/command_demo.csv。
V2_DEMO_STATS = {
    (0.0, 0.0, 0.0): {"vx_mean": -0.024, "vx_std": 0.065, "wz_mean": -0.061, "wz_std": 0.621, "z_std": 0.0037},
    (0.5, 0.0, 0.0): {"vx_mean": 0.517, "vx_std": 0.069, "wz_mean": 0.039, "wz_std": 0.781, "z_std": 0.0045},
    (0.0, 0.0, 0.5): {"vx_mean": -0.036, "vx_std": 0.063, "wz_mean": 0.301, "wz_std": 0.554, "z_std": 0.0045},
    (-0.3, 0.0, 0.0): {"vx_mean": -0.303, "vx_std": 0.055, "wz_mean": -0.371, "wz_std": 0.575, "z_std": 0.0038},
}
V2_DEMO_TERMINATIONS = 0

# v2 最佳策略前进 6 秒的四脚接触统计（触地占比 %、抬脚次数），
# 用于对照钉腿惩罚的效果；后左腿 91.6% 触地即"三腿跛行"问题。
V2_GAIT_STATS = [
    ("前右 FR", 46.9, 27),
    ("前左 FL", 52.0, 24),
    ("后右 RR", 57.8, 38),
...
def write_report(
    ...
    comparison_lines.append(
        ...
        f"{V2_DEMO_TERMINATIONS} | {len(demo_terminations)} |"
    )

    symmetry_lines = []
    for (label, v2_duty, v2_lifts), (_, duty, lifts) in zip(
        V2_GAIT_STATS, symmetry_rows,
    ):
        symmetry_lines.append(
            f"| {label} | {v2_duty:.1f}% / {v2_lifts} | {duty:.1f}% / {lifts} |"
        )
    termination_text = (
        ", ".join(f"t={float(row['time_s']):.2f}s" for row in demo_terminations)
        if demo_terminations else "无"
    )
    report = f"""# Pupper PPO 本机训练报告（v2.1）
