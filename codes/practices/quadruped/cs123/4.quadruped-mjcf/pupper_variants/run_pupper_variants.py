"""运行 Pupper 结构变体生成、站姿搜索与 PD 参数扫描。

脚本从一份共享骨架派生 original、long-leg 和 heavy 三种模型，
分别搜索站姿、扫描 PD 参数，并生成对比图与数值结果。
"""
...
from utils import PDGains, apply_theme, fit_scene_frame, load_font, pd_heatmap


HERE = Path(__file__).resolve().parent
MODEL_DIR = HERE.parent / "outputs" / "pupper_variants" / "models"
OUTPUT_DIR = HERE.parent / "outputs" / "pupper_variants"
SKELETON_PATH = HERE / "skeleton.xml"
...
def _read_skeleton_template() -> ET.Element:
    ...
    if "variant_thigh" not in names or "variant_calf" not in names:
        raise AssertionError("pupper_variants/skeleton.xml 缺少 variant_thigh / variant_calf 注入点")
    return root
...
def make_variant(name: str, *, leg_scale: float = 1.0, torso_mass_scale: float = 1.0) -> Path:
  ...
  <compiler angle=\"radian\" meshdir=\"../../../../assets/mjcfs/meshes/stl/\" autolimits=\"true\"/>
  <default>
...
def save_heatmap(results: dict[str, VariantResult], path: Path) -> None:
    ...
    fig.suptitle("Pupper 变体：三只 Pupper 的 PD 参数扫描")
...
def save_sweep_pickle(results: dict[str, VariantResult], path: Path) -> None:
    """把 16 格 heatmap 数据 pickle 到输出目录，方便复查。"""
...
def main() -> None:
    ...
    save_heatmap(results, OUTPUT_DIR / "pd_heatmap.png")
    save_zoo_still(results, OUTPUT_DIR / "pupper_zoo.png")
    save_stand_z_plot(results, OUTPUT_DIR / "stand_z_vs_t.png")
    save_sweep_pickle(results, OUTPUT_DIR / "pd_stable_gains.pkl")

    print(f"Pupper 变体观察图已写入 {OUTPUT_DIR}/")
    for key in ("original", "longleg", "heavy"):
        result = results[key]
        print(
