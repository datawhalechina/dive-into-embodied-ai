...
[`6.rl_pupper_v2_1`](../6.rl_pupper_v2_1/) 的增量迭代：**仅新增动作低通滤波**（策略输出先过一阶 EMA，α=0.5，50 Hz 下截止约 5.5 Hz，再进 PD 伺服），其余与 v2.1 完全一致。

动机：v2.1 的残余抖动来自网络每 20 ms 输出一次新目标；手写开环步态（`../5.gait-control/`）"天然平滑"的本质是动作频带受限的周期轨迹。滤波把同样的频带先验加给 RL——保留步态基频与摆动相、滤掉高频抖动，同时保住 RL 的闭环平衡与抗扰能力（这也是 RL 相对纯 gait control 的方法优势所在）。训练与评估都经过滤波，真机部署可原样保留。

版本关系：[`6.rl_pupper`](../6.rl_pupper/)（v1 基线）→ [`6.rl_pupper_v2`](../6.rl_pupper_v2/)（观测/奖励/课程全面优化）→ [`6.rl_pupper_v2_1`](../6.rl_pupper_v2_1/)（钉腿惩罚修三腿跛行）→ 本目录（动作滤波治高频抖动）。本版两阶段全部带滤波从零重训。
...
```bash
uv run pytest -q 6.rl_pupper_v2_2/tests
uv run python 6.rl_pupper_v2_2/pupper_env.py
```
...
```bash
uv run python 6.rl_pupper_v2_2/train.py --config 6.rl_pupper_v2_2/configs/bootstrap_walk.yaml
uv run python 6.rl_pupper_v2_2/train.py --config 6.rl_pupper_v2_2/configs/smooth_finetune.yaml
```
...
```bash
uv run python 6.rl_pupper_v2_2/train.py
```

先用短训练确认流程正常：

```bash
uv run python 6.rl_pupper_v2_2/train.py --timesteps 100000 --n-envs 4
```
...
```bash
uv run python 6.rl_pupper_v2_2/train.py --config 6.rl_pupper_v2_2/configs/train.yaml
```
...
```bash
uv run python 6.rl_pupper_v2_2/train.py \
  --config 6.rl_pupper_v2_2/configs/finetune_gait.yaml
```
...
MuJoCo 仿真仍在 CPU 上运行。默认配置使用 `device: cpu`；Apple Silicon 可改成 `mps`，或临时传入 `--device mps`。SB3 的 `auto` 在没有 CUDA 时会回退 CPU，不会自动选择 MPS。训练产物写入 `6.rl_pupper_v2_2/outputs/pupper_ppo.zip`。

启用 TensorBoard 后，在另一个终端查看曲线：

```bash
uv run tensorboard --logdir 6.rl_pupper_v2_2/outputs/tb
```
...
uv run python 6.rl_pupper_v2_2/train.py \
  --timesteps 20000000 \
  --checkpoint 6.rl_pupper_v2_2/outputs/pupper_ppo.zip
```
...
```bash
uv run python 6.rl_pupper_v2_2/evaluate.py
```

评估会生成：

- `6.rl_pupper_v2_2/outputs/demo.gif`
- `6.rl_pupper_v2_2/outputs/velocity_tracking.webp`
