...
# 上一版（45 维观测、固定命令）演示的平稳度基线，口径与 demo_phase_stats 一致；
# 原始数据见 git 历史中的 raw/command_demo.csv。
V1_DEMO_STATS = {
...
def write_report(
    ...
    forward_v2 = phase_stats[(0.5, 0.0, 0.0)]
    comparison_lines.append(
        f"| 前进 `vx=0.5` | 机身高度波动 | "
    ...
    comparison_lines.append(
        f"| 演示全程 | 终止次数 | {V1_DEMO_TERMINATIONS} | {len(demo_terminations)} |"
    )
    termination_text = (
