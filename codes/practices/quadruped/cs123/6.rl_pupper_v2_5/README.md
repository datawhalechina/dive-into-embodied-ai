...
[`6.rl_pupper_v2_4`](../6.rl_pupper_v2_4/) 的**纯配置迭代**（代码逐字节一致）：稳定性打磨。v2.4 的摆动整形让步子变大变慢后机身晃动回升，本版从 v2.4 的 7M checkpoint 续训 8M 步，只通过 `reward_overrides` 收紧稳定性——`lin_vel_z` 恢复满额 -2.0、`ang_vel_xy` 加倍到 -0.1、`tracking_lin_vel` 提到 2.0，熵降至 0.0002 收尾（`configs/stability_polish.yaml`）。

版本关系：[`6.rl_pupper`](../6.rl_pupper/)（v1）→ [`v2`](../6.rl_pupper_v2/)→ [`v2_1`](../6.rl_pupper_v2_1/)→ [`v2_2`](../6.rl_pupper_v2_2/)→ [`v2_3`](../6.rl_pupper_v2_3/)→ [`v2_4`](../6.rl_pupper_v2_4/)→ 本目录。
...
```bash
uv run pytest -q 6.rl_pupper_v2_5/tests
uv run python 6.rl_pupper_v2_5/pupper_env.py
```

## 训练策略

从 v2.4 的归档 checkpoint 续训稳定性打磨阶段（前四阶段复用 v2.2–v2.4 产物）：

```bash
uv run python 6.rl_pupper_v2_5/train.py --config 6.rl_pupper_v2_5/configs/stability_polish.yaml
```

命令行参数会覆盖 YAML 中 `run` 下的对应字段。正式训练可直接修改 YAML，也可以临时覆盖：

```bash
uv run python 6.rl_pupper_v2_5/train.py \
  --timesteps 20000000 \
  --n-envs 8 \
  --tensorboard
```
...
```bash
uv run python 6.rl_pupper_v2_5/train.py --config 6.rl_pupper_v2_5/configs/train.yaml
```
...
```bash
uv run python 6.rl_pupper_v2_5/train.py \
  --config 6.rl_pupper_v2_5/configs/finetune_gait.yaml
```
...
MuJoCo 仿真仍在 CPU 上运行。默认配置使用 `device: cpu`；Apple Silicon 可改成 `mps`，或临时传入 `--device mps`。SB3 的 `auto` 在没有 CUDA 时会回退 CPU，不会自动选择 MPS。训练产物写入 `6.rl_pupper_v2_5/outputs/pupper_ppo.zip`。

启用 TensorBoard 后，在另一个终端查看曲线：

```bash
uv run tensorboard --logdir 6.rl_pupper_v2_5/outputs/tb
```
...
uv run python 6.rl_pupper_v2_5/train.py \
  --timesteps 20000000 \
  --checkpoint 6.rl_pupper_v2_5/outputs/pupper_ppo.zip
```
...
```bash
uv run python 6.rl_pupper_v2_5/evaluate.py
```

评估会生成：

- `6.rl_pupper_v2_5/outputs/demo.gif`
- `6.rl_pupper_v2_5/outputs/velocity_tracking.webp`
