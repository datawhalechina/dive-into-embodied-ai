...
def test_action_filter_smooths_motor_targets():
    env = PupperEnv(perturb_enabled=False, cmd_resample_steps=0)
    env.reset(seed=0)
    # 第一步满幅动作只有 alpha 比例进入电机目标
    env.step(np.ones(12, np.float32))
    assert np.allclose(env.filtered_action, env.action_filter_alpha)
    # 交替满幅正负动作时，滤波后的幅值应明显小于原始动作
    for sign in (-1.0, 1.0, -1.0, 1.0):
        env.step(np.full(12, sign, np.float32))
    assert np.all(np.abs(env.filtered_action) < 0.8)


def test_action_filter_alpha_one_is_passthrough():
    env = PupperEnv(
        perturb_enabled=False, cmd_resample_steps=0, action_filter_alpha=1.0,
    )
    env.reset(seed=0)
    action = np.full(12, 0.3, np.float32)
    env.step(action)
    assert np.allclose(env.filtered_action, 0.3, atol=1e-6)


def test_foot_clearance_zero_when_standing():
    env = PupperEnv(perturb_enabled=False, cmd_resample_steps=0)
    env.reset(seed=0)
    env.cmd = np.array([0.5, 0.0, 0.0], dtype=np.float32)
    # 零动作站立：四足全触地，无摆动相，不产生 clearance 惩罚
    for _ in range(30):
        _, _, _, _, info = env.step(np.zeros(12, np.float32))
    assert info["r_foot_clearance"] == 0.0


def test_feet_stance_time_penalizes_planted_feet_under_command():
    env = PupperEnv(perturb_enabled=False, cmd_resample_steps=0)
    env.reset(seed=0)
    env.cmd = np.array([0.5, 0.0, 0.0], dtype=np.float32)
    info = {}
    # 零动作站立 1 秒：四足持续触地超过 STANCE_TIME_LIMIT 后开始罚分
    for _ in range(50):
        _, _, _, _, info = env.step(np.zeros(12, np.float32))
    assert info["r_feet_stance_time"] < 0.0

    env.cmd = np.array([0.0, 0.0, 0.0], dtype=np.float32)
    _, _, _, _, info = env.step(np.zeros(12, np.float32))
    assert info["r_feet_stance_time"] == 0.0
