...
REQUIRED_CONFIG_KEYS = {
    ...
    "ppo": (
        ...
        "gamma", "gae_lambda", "clip_range", "ent_coef", "net_arch", "verbose",
    ),
...
def transfer_policy_weights(target: PPO, source_path: Path, device: str) -> None:
    """把 45 维策略迁移到 50 维策略，新增输入权重初始化为零。"""
...
def train(config: dict[str, Any]) -> Path:
    ...
    vec_env = SubprocVecEnv([
        make_env(seed, rank, max_steps, gait_config) for rank in range(n_envs)
    ])
    try:
        ...
        if checkpoint:
            ...
            if transfer_checkpoint:
                print(f"从 45 维策略迁移参数：{transfer_checkpoint}")
