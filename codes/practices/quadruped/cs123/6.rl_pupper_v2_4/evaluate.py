...
# 慢速巡航：与 5.gait-control 演示同一速度档（其 weld_speed 为 0.04–0.1 m/s），
# 用于同速对比观感；标准序列保留用于历版指标对照。
CRUISE_SCRIPT = [
    ((0.0, 0.0, 0.0), 1.5),
    ((0.15, 0.0, 0.0), 6.0),
    ((0.0, 0.0, 0.0), 1.5),
...
# 相机跟随做一阶平滑：机身的每次微晃不再被相机放大成整个画面的抖动。
CAMERA_SMOOTHING = 0.12


def _cmd_at(time_s: float, script=CMD_SCRIPT):
    elapsed = 0.0
    for command, duration in script:
...
def render_demo(model, env, out_path: Path, script=CMD_SCRIPT) -> None:
    total_seconds = sum(duration for _, duration in script)
    ...
    frames = []
    next_frame_time = 0.0
    smoothed_lookat = env.data.xpos[env._base_id].copy()
    try:
        for step_index in range(n_steps):
            time_s = step_index * env.dt
            env.cmd = np.array(_cmd_at(time_s, script), dtype=np.float32)
            action, _ = model.predict(obs, deterministic=True)
            obs, _, terminated, truncated, _ = env.step(action)
            smoothed_lookat += CAMERA_SMOOTHING * (
                env.data.xpos[env._base_id] - smoothed_lookat
            )
...
def main() -> None:
    ...
    # 本版策略是 trot 条件化的 53 维策略：评估固定 trot、关闭步态切换、
    # 关闭命令重采样和随机扰动。
    env = PupperEnv(
    ...
    render_demo(model, env, output_dir / "cruise_demo.gif", script=CRUISE_SCRIPT)
