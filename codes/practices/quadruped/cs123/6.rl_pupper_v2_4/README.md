...
[`6.rl_pupper_v2_3`](../6.rl_pupper_v2_3/) 的增量迭代：**摆动整形**，针对"步频太快、看不到抬腿"两点观感。

- `AIR_TIME_TARGET` 0.1 → 0.25 秒：短促点地在触地时扣分，步频从约 5 Hz 落向 trot 时钟的 2 Hz；
- 新增 `foot_clearance` 奖励（目标 3.5cm，对齐 `../5.gait-control/` 的 step_height）：让"抬大腿"有利可图——此前贴地掠过是能量最优解（v2.2 实测摆动高度仅 1.3cm）；
- 新增 0.15 m/s 慢速巡航演示：手写演示前进速度只有 0.04–0.1 m/s，与 0.5 m/s 的标准演示不同速档，同速对比才公平。

训练从 v2.3 的 5M checkpoint 同结构续训 8M 步（`configs/finetune_gait.yaml`）。注意本目录环境的 `AIR_TIME_TARGET` 与 `foot_clearance` 常量和 v2.2/v2.3 不同，若从零复现前两阶段，结果会与复用产物略有差异。

版本关系：[`6.rl_pupper`](../6.rl_pupper/)（v1）→ [`6.rl_pupper_v2`](../6.rl_pupper_v2/)→ [`6.rl_pupper_v2_1`](../6.rl_pupper_v2_1/)→ [`6.rl_pupper_v2_2`](../6.rl_pupper_v2_2/)→ [`6.rl_pupper_v2_3`](../6.rl_pupper_v2_3/)→ 本目录。
...
```bash
uv run pytest -q 6.rl_pupper_v2_4/tests
uv run python 6.rl_pupper_v2_4/pupper_env.py
```

## 训练策略

从 v2.3 的归档 checkpoint 续训第四阶段（前三阶段复用 v2.2 / v2.3 产物）：

```bash
uv run python 6.rl_pupper_v2_4/train.py --config 6.rl_pupper_v2_4/configs/finetune_gait.yaml
```

先用短训练确认流程正常：

```bash
uv run python 6.rl_pupper_v2_4/train.py --timesteps 100000 --n-envs 4
```

命令行参数会覆盖 YAML 中 `run` 下的对应字段。正式训练可直接修改 YAML，也可以临时覆盖：

```bash
uv run python 6.rl_pupper_v2_4/train.py \
  --timesteps 20000000 \
  --n-envs 8 \
  --tensorboard
```
...
```bash
uv run python 6.rl_pupper_v2_4/train.py --config 6.rl_pupper_v2_4/configs/train.yaml
```
...
```bash
uv run python 6.rl_pupper_v2_4/train.py \
  --config 6.rl_pupper_v2_4/configs/finetune_gait.yaml
```
...
MuJoCo 仿真仍在 CPU 上运行。默认配置使用 `device: cpu`；Apple Silicon 可改成 `mps`，或临时传入 `--device mps`。SB3 的 `auto` 在没有 CUDA 时会回退 CPU，不会自动选择 MPS。训练产物写入 `6.rl_pupper_v2_4/outputs/pupper_ppo.zip`。

启用 TensorBoard 后，在另一个终端查看曲线：

```bash
uv run tensorboard --logdir 6.rl_pupper_v2_4/outputs/tb
```
...
uv run python 6.rl_pupper_v2_4/train.py \
  --timesteps 20000000 \
  --checkpoint 6.rl_pupper_v2_4/outputs/pupper_ppo.zip
```
...
```bash
uv run python 6.rl_pupper_v2_4/evaluate.py
```

评估会生成：

- `6.rl_pupper_v2_4/outputs/demo.gif`
- `6.rl_pupper_v2_4/outputs/velocity_tracking.webp`
