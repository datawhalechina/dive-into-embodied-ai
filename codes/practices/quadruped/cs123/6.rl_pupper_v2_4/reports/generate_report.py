...
RUN_DIR = LAB_DIR / "outputs" / "trot_finetune_8m"
...
def save_run_config() -> dict:
    ...
    with (LAB_DIR / "configs" / "finetune_gait.yaml").open(encoding="utf-8") as file:
        finetune_config = yaml.safe_load(file)
    for name in ("bootstrap_walk.yaml", "smooth_finetune.yaml", "finetune_gait.yaml"):
...
# v2.3（trot 条件化，无摆动整形）演示基线，
# 数据来自 ../6.rl_pupper_v2_3/reports/raw/command_demo.csv。
V23_DEMO_STATS = {
    (0.0, 0.0, 0.0): {"vx_mean": -0.009, "vx_std": 0.050, "wz_mean": -0.053, "wz_std": 0.336, "z_std": 0.0033},
    (0.5, 0.0, 0.0): {"vx_mean": 0.525, "vx_std": 0.047, "wz_mean": 0.024, "wz_std": 0.406, "z_std": 0.0035},
    (0.0, 0.0, 0.5): {"vx_mean": 0.004, "vx_std": 0.048, "wz_mean": 0.460, "wz_std": 0.400, "z_std": 0.0029},
    (-0.3, 0.0, 0.0): {"vx_mean": -0.322, "vx_std": 0.034, "wz_mean": -0.064, "wz_std": 0.336, "z_std": 0.0062},
}
V23_DEMO_TERMINATIONS = 0
...
def write_report(
    ...
    comparison_lines.append(
        ...
        f"{V23_DEMO_TERMINATIONS} | {len(demo_terminations)} |"
    )

    symmetry_lines = []
    for (label, v2_duty, v2_lifts), (_, v23_duty, v23_lifts), (_, duty, lifts) in zip(
        V2_GAIT_STATS, V23_GAIT_STATS, symmetry_rows,
    ):
        symmetry_lines.append(
            f"| {label} | {v2_duty:.1f}% / {v2_lifts} | "
            f"{v23_duty:.1f}% / {v23_lifts} | {duty:.1f}% / {lifts} |"
        )
    termination_text = (
        ", ".join(f"t={float(row['time_s']):.2f}s" for row in demo_terminations)
        if demo_terminations else "无"
    )
    report = f"""# Pupper PPO 本机训练报告（v2.4）
...
在 Apple M3 Ultra 上使用 16 个 CPU MuJoCo 环境，在 v2.3 产物之上完成第四阶段摆动整形微调 {final_step:,} 步（耗时约 {duration_minutes:.1f} 分钟）。

在摆动整形收敛（≥6M）的 checkpoint 中按三档速度平均 MAE 加摔倒惩罚选择，最佳为 **[{best_steps / 1e6:g}M 步](raw/checkpoints/pupper_ppo_{best_steps}_steps.zip)**，平均 MAE 为 {best_summary['mean_mae']:.3f} m/s，评估重置次数为 {best_summary['total_resets']}。
...
- 阶段四摆动整形：{config['run']['total_timesteps']:,} 步，从 v2.3 的 5M checkpoint 续训，新奖励生效（配置见 [`raw/finetune_gait.yaml`](raw/finetune_gait.yaml)）
...
- trot 接触匹配率 **{contact_match * 100:.1f}%**（随机步态基线约 50%；v2.3 最佳为 55.7%）
- 平均步频 **{step_hz:.1f} Hz**（v2.3 约 4.9 Hz；trot 时钟为 2 Hz）
- 摆动相足端离地高度：均值 **{swing_mean_cm:.1f}cm** / P90 **{swing_p90_cm:.1f}cm**（v2.2 基线 1.3 / 2.1cm；目标 3.5cm）
...
def main() -> None:
    ...
    # 本版目标是摆动整形：纯 MAE 会偏向尚未吸收新奖励的最早期 checkpoint，
    # 因此在整形收敛（>= 6M）的 checkpoint 中按 MAE + 摔倒惩罚选择。
    converged = [
        row for row in aggregate if row["checkpoint_steps"] >= 6_000_000
    ]
    best = min(converged or aggregate, key=lambda row: row["score"])
