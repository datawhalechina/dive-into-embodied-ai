"""Render training reward curves from metrics JSONL files as one SVG.

Same conventions as make_charts.py: inline SVG, colors from CSS custom
properties, series assigned palette slots in argument order.

    uv run python benchmarks/make_reward_chart.py --out curves.svg \
        "R9700 (ROCm) seed 0=rocm-s0/metrics.jsonl" "A40 seed 0=a40-s0/metrics.jsonl"
"""
...
def curve_chart(series, title, subtitle) -> str:
  xmax = max(r["step"] for _, rows in series for r in rows)
  ys = [r[REWARD_KEY] for _, rows in series for r in rows]
  ...
  ends = _spread([y(rows[-1][REWARD_KEY]) for _, rows in series])
...
def main() -> None:
  ...
  p.add_argument("series", nargs="+", help="LABEL=path/metrics.jsonl")
  p.add_argument("--out", required=True)
  p.add_argument("--title", default="训练回报曲线")
  p.add_argument("--subtitle", default="同一任务、同一套超参数，只是换了显卡。")
  a = p.parse_args()
  ...
  for spec in a.series:
    ...
    loaded.append((label, [r for r in rows if REWARD_KEY in r]))

  svg = curve_chart(loaded, a.title, a.subtitle)
