"""Measure batched env.step throughput for our Go2 velocity task.

Same runner and JSON schema as env_throughput.py (one child process per batch
size, periodic queue drain), but stepping unitree_rl_mjx's Go2VelocityFlat —
the exact environment the full training runs pay for, feet-only collision
model included. Actions are a fixed random batch; no mid-run resets.

    uv run python benchmarks/go2_env_throughput.py --out results.json
"""
...
STEPS = 100
# Same queue-drain cadence as env_throughput.py (ROCm HIP segfault workaround,
# run on every backend so numbers stay comparable).
SYNC_EVERY = 10
...
def _measure_one(num_envs: int, steps: int) -> None:
  ...
  manifest = {
    "measures": "env_step (physics + observations + rewards), Go2VelocityFlat",
...
def main() -> None:
  ...
  for num_envs in BATCH_SIZES:
    if num_envs > args.max_batch:
      break
    proc = subprocess.run(
      [sys.executable, __file__, "--num-envs", str(num_envs), "--steps",
       str(args.steps)],
      capture_output=True,
      text=True,
      check=False,
    ...
    if line is None:
      print(f"{num_envs:6d} envs  FAILED (exit {proc.returncode})", flush=True)
    ...
    r = payload["result"]
    results.append(r)
    vram = f"  vram {r['vram_mb']:,.0f} MB" if r["vram_mb"] else ""
    print(
      f"{num_envs:6d} envs  compile {r['compile_s']:6.1f}s"
      f"  {r['steps_per_s']:12,.0f} env steps/s{vram}",
      flush=True,
    )

  if args.out and manifest is not None:
