...
5.2 节解决的是**节奏**问题——每条腿什么时候踩地、什么时候抬腿。但腿在空间中沿什么样的曲线运动，还没有说清楚。本节补上这一块。
...
把这两段衔接起来，单条腿一个周期内的足端轨迹就是一条**上下拼接的闭合回路**：下半段贴地运动（stance，前→后），上半段在空中划弧（swing，后→前）。CS123 Lab 4 slides 在这条轨迹上标出了四个关键点，如 [图 3](#fig-foot-trajectory-phases) 所示。
...
```python
from ik_pupper import ik_leg_3dof   # 第 3 章解析法 IK 的四足版本
...
def gait_step(t, T_cycle, step_length, step_height, stand_height):
    ...
    for k, leg in enumerate(['FL', 'FR', 'RL', 'RR']):
        ...
        # y(横向)由 HAA 关节决定,这里直接锁在 stance 站姿下的横向偏置
        # LEG_Y_OFFSET 通常来自 URDF/MJCF 中各腿髋关节的 y 安装位置
...
图 6 由 `codes/practices/quadruped/cs123/5.gait-control/render_gait_experiment_gifs.py` 在 conda 的 `mujoco` 环境中生成，加载的是本章目录下的 `pupper_gait_demo.xml`。这个 XML 延续第 4 章的 `base`、`FL/FR/RL/RR`、`HAA/HFE/KFE` 和 12 actuator 命名，但用 capsule / sphere 几何直接显示腿链和足端，避免 mesh 与教学 IK 简化模型错位。
