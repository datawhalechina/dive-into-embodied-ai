...
- [多模态 · ViT 与视觉表征](/docs/foundations/vlm-vla-world-model/vit_and_visual_representation)（可选延伸）
...
- **`fovy` 60° 是仿真版的好默认值**。真机 Pupper 用的是鱼眼镜头（视场角 150°+，但带强烈枕形畸变），所以 Lab 7 slides 里**专门给了一段 unwarping 代码**——仿真版直接用 pinhole 模型跳过这一步，等迁移到真机时再补。
...
> **怎么选**：仿真里专追一个目标用 HSV，写一行 `cv2.inRange` 调试方便、确定性高。一旦想做"找人 / 找椅子 / 找猫"，YOLO 是没得选的。Lab 7 slides 用的是 YOLO + ROS message，本质是一回事：`results.boxes` 对应 slides 里的 `msg.detections.data`。

## 8.4 图像坐标 → 航向角：P 控制器

目标在画面里偏左，机器人就该向左转——这就是 Lab 7 slides 反复强调的**P 控制器**。先把"偏多少像素"归一化成 [-1, 1]：
...
8.4 节的 P 控制器有两个工程缺口，Lab 7 slides 都点到了：
...
录一段 30 秒视频是组队任务交付物，**视频里要能看到至少一次"丢失 → 找回"切换**，这是本章最有信息量的一帧。
...
更强的玩法是**把整张 RGB 图作为 user message 发给 Claude**，让它用视觉直接判断。这一步把"object detector + LLM"两段融成 VLM 一段，对应 Lab 7 slides 的 **"multimodal AI system combining camera feeds with language models"**：
...
§8.3–8.5 分别做了 HSV 检测、P 控制器和 TrackerFSM，§8.6–8.7 在原地和行走两种模式下验证了追踪效果。组队 Lab 8 把这三块拧到一起做"毕业汇演"——Pupper 看到红球、走过去、在球前停下，20 秒追球 GIF 是整门课唯一一段"前面所有 Lab 的资产同时上场"的视频：Lab 5 的上游 RTNeural policy 跑底层步态，Lab 7 的物理循环（50 Hz policy / 500 Hz physics）驱动关节，这一章的视觉闭环在上面叠了 10 Hz 的 camera → detect → FSM → (vx, wz) 控制层。

<div align="center">

![Lab 8 teaser：HSV 检测 + P 控制器 + RL 步态追球](figs/lab8_ball_chase.gif)

*追球毕业视频：红球每 3 秒随机移动一次，Pupper 通过头部相机的 HSV 检测定位球心，P 控制器把像素偏差转成航向角速度 wz，TrackerFSM 在 SEARCHING / TRACKING / STOPPED 三态间切换。字幕栏实时显示当前 FSM 状态和速度命令，底层物理和 Lab 5 / Lab 7 完全相同——区别只在于谁在发 (vx, wz)：§7.3 是 LLM 从自然语言推理出来的，这里是视觉闭环从像素算出来的。*
...
完整 starter / 测试 / 交付清单见 [`team_learning/lab_8_ball_chase/`](https://github.com/robots-hub/cs123-simulation/tree/main/team_learning/lab_8_ball_chase)（cs123-simulation 私有仓库，组员请确认 GitHub 账号已加权限）。
