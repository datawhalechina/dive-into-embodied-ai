...
- 交互页：IK Playground（施工中，链接稍后补上），鼠标拖目标就能看 DLS 的收敛
...
:::tip 想立刻看效果?
不想装 MuJoCo 可以打开 IK Playground（施工中，链接稍后补上），鼠标拖目标就能看 DLS 的收敛过程——尤其是拖到机械臂"伸直"那条奇异线附近，能直观感受 $\lambda$ 在做什么。页面里跑的就是上面这个 `ik_dls`，只是搬到浏览器里、不挂物理引擎。
:::
