...
sidebar_position: 3
title: 3. 逆运动学
description: "从末端目标位姿反解关节角：解析法、数值法，以及在 MuJoCo 里做到点控制。"
---

# 3. 逆运动学


正运动学是单向的：关节角 → 末端。逆运动学是我们**真正想要的方向**：给一个末端目标，求出关节角。这一章我们用两种方式实现 IK，并把它接到第 1 章的 PD 控制器上，做出第一个"到点控制" demo。
...
- [ROS · 正逆运动学实现](/docs/foundations/robotics-basics/fk_ik_implementation)
- 交互页:[IK Playground](https://robotshub.cn/cs123/ik-playground)，鼠标拖目标就能看 DLS 的收敛
...
:::tip 想立刻看效果?
不想装 MuJoCo 可以直接打开 [IK Playground](https://robotshub.cn/cs123/ik-playground)，鼠标拖目标就能看 DLS 的收敛过程——尤其是拖到机械臂"伸直"那条奇异线附近，能直观感受 $\lambda$ 在做什么。页面里跑的就是上面这个 `ik_dls`，只是搬到浏览器里、不挂物理引擎。
:::
...
光看末端轨迹不够直观——把**目标点（绿球）**和**当前末端（红球）**都叠到 MuJoCo viewer 里，DLS 的"追"才能一眼看出来。这一段是第 2 章 viewer 实验的直接延伸：
...
§3.8 让平面臂走了个抽象三角形，§3.9 又把绿球（目标）和红球（当前末端）叠到了 viewer 里。组队 Lab 3 把这两件事各往物理意义上推一格：三角形的三个顶点变成 **touchdown / standing / liftoff**，中间补一段 **mid-swing 抛物线**——这就是 **Raibert 三角**，四足所有步态的原子动作。轨迹接到 Lab 2 那条 Pupper 真腿（用 `<weld>` 把 hip 挂在空中），DLS IK + Lab 1 的 PD 闭环跑起来。Lab 5 的 trot 步态就是"这条腿 × 4 + 相位偏移"，提前在这里把原子动作打磨好。

<div align="center">

![Lab 3 teaser：Pupper 单腿 Raibert 悬空踏步](figs/lab3_leg_stepping.gif)
...
- [ ] Stretch：`step_length` 从 0 线性扫到 0.12 m，看"原地踏步"渐变成"D 字形迈步"——给 Lab 5 trot 留个伏笔

完整 starter / 测试 / 交付清单见 [`team_learning/lab_3_stepping/`](https://github.com/robots-hub/cs123-simulation/tree/main/team_learning/lab_3_stepping)（cs123-simulation 私有仓库，组员请确认 GitHub 账号已加权限）。
