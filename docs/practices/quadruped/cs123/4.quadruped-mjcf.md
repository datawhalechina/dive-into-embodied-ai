...
[§4.5](/docs/practices/quadruped/cs123/quadruped-mjcf#45-让-pupper-站起来) 和 [§4.6](/docs/practices/quadruped/cs123/quadruped-mjcf#46-调-gainprm-与-biasprm) 调的是一只已经搭好的 Pupper。这个 Lab 往前推一步：改出 `original` / `long-leg` / `heavy` 三只变体，再分别求站姿、扫 PD 甜点，并把结果做成图片交付。
...
| 文件 | 作用 |
| --- | --- |
| `starter_todo.py` | 学生起点，只有 3 个算法 TODO |
| `starter.py` | 参考实现/完整版本，卡住时可以对照思路 |
| `tests.py` | 数值验收：变体能编译、long-leg 比例正确、三只都能稳定站住 |
| `make_artifacts.py` | 生成交付物：heatmap、三联静帧、base z 曲线 |
| `shared/models/skeleton.xml` | 三只变体共用的骨架，Lab 里不要复制整棵 body 树 |

如果你的练习环境只保留 TODO 版，就直接补 `starter.py` 里的三处空白；如果像当前仓库一样同时有 `starter_todo.py` 和 `starter.py`，学习时先读 `starter_todo.py` 的接口，再用 `starter.py` 对照完整流程。
...
pd_sweet_spot()
  ...
  ↓
make_artifacts.py
  保存 heatmap、三只 Pupper 并排图、base z 曲线
```
...
评分用的是最后 1 秒 `base z` 的标准差。只看最终高度不够，因为有些参数会让机器人"趴得很稳"，但那不是站住。

### 你最终交什么

跑完 `make_artifacts.py` 后，重点看 `portfolio/` 目录：

```text
portfolio/deliverable.png      三只 Pupper 的 4×4 PD heatmap
portfolio/pupper_zoo.png       original / long-leg / heavy 三联站姿图
portfolio/stand_z_vs_t.png     三只调好后 base z 时间曲线
portfolio/pd_sweet_spots.pkl   扫描数据，方便复查
```

其中 `pupper_zoo.png` 长这样：

<div align="center">

![Ch4 成果预览:三只 Pupper 各自找甜点站住](figs/lab4_pupper_zoo.webp)

*左 original，中 long-leg（thigh + calf × 1.5，比原版高约 10 cm），右 heavy（torso 质量加大，背上深灰小盒是配重视觉标记）。三只都用各自扫出来的 `(Kp, Kd)` 站稳；评分时看最后 1 秒 base z 标准差是否小于 5 mm。*

</div>

`deliverable.png` 则是 4×4 heatmap，用来对比不同身体参数会把 PD 甜点推到哪个区域。
...
# 先看 TODO 提示，确认要补哪三个函数
uv run python lab_4_urdf_surgery/starter_todo.py

# 补完后跑测试；当前仓库的 tests.py 针对 starter.py，课程发放版通常会让你补同名 starter.py
uv run python lab_4_urdf_surgery/tests.py

# 测试通过后生成图片交付物
uv run python lab_4_urdf_surgery/make_artifacts.py
```

如果你只是想先看参考结果，可以运行当前仓库里的完整版本：

```bash
uv run python lab_4_urdf_surgery/starter.py
```

正常跑通时，终端会打印三只 Pupper 的最佳 `(Kp, Kd)` 和最后 1 秒 `z_std`，`portfolio/` 里会出现三张图。

### 验收标准

`tests.py` 主要检查这些点：

- long-leg 的 thigh 长度大约是 original 的 1.5 倍。
- 三份变体都 `<include>` 同一个 `shared/models/skeleton.xml`，不要复制 body 树。
- heavy 的总质量明显大于 original。
- original 的甜点不应该选太软的格子，`Kp >= 30` 且 `Kd >= 1.0`。
- 三只 Pupper 用各自甜点站立时，最后 1 秒 base z 标准差 `< 5 mm`。

常见坑基本都在这几条里：foot site 忘了跟着 calf 变长、把 base 的 qpos/qvel 当成关节拿去 PD、三只模型直接 include 导致名字冲突、只看最终高度不看 z 抖动。

完整 starter、测试和交付脚本见 [`exercises/lab_4_urdf_surgery/`](https://github.com/datawhalechina/dive-into-embodied-ai/tree/master/codes/practices/quadruped/cs123/exercises/lab_4_urdf_surgery)。
