...
:::tip macOS 用户：用 `mjpython` 启动 viewer
macOS 上 GUI 事件循环必须运行在主线程，直接 `python -m mujoco.viewer` 会报错或卡死。`pip install mujoco` 时会顺带装一个 `mjpython` 可执行文件，它就是为这个限制准备的 Python 启动器——把所有带 viewer 的命令里的 `python` 全部换成 `mjpython` 即可：

```bash
mjpython -m mujoco.viewer
mjpython -m mujoco.viewer --mjcf=hello.xml
```

Linux 和 Windows 不受这个限制，继续用 `python` 就好。后面 3.5 节里所有带 viewer 的脚本同样适用这条规则。
:::
