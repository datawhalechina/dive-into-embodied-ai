...
const playgrounds = [
  {number: '01', title: translate({message: "调好第一个控制器"}), subtitle: translate({message: "PD 控制"}), description: translate({message: "调整增益，观察响应、超调与稳定性的变化。"}), to: '/cs123/pd-playground', icon: SlidersHorizontal},
  {number: '02', title: translate({message: "看懂关节如何运动"}), subtitle: translate({message: "正运动学"}), description: translate({message: "转动关节，观察足端位置与坐标系的关系。"}), to: '/cs123/fk-playground', icon: Move3D},
  {number: '03', title: translate({message: "让足端走到目标点"}), subtitle: translate({message: "逆运动学"}), description: translate({message: "移动目标点，探索机器人腿的可达空间。"}), to: '/cs123/ik-playground', icon: Crosshair},
];
      ...
      <div className={styles.heroContent}>
        <div className={styles.heroCopy}>
          ...
          <div className={styles.heroNotes} aria-label={translate({message: "课程特色"})}>
            <span><BookOpen size={14} aria-hidden="true" />{translate({message: "中文教程"})}</span>
            <span><Code2 size={14} aria-hidden="true" />{translate({message: "可运行代码"})}</span>
            <span><GitFork size={14} aria-hidden="true" />{translate({message: "社区共建"})}</span>
          </div>
        </div>
      </div>
      <Link className={styles.robotSpotlight} to="/docs/practices/humanoid/microduck-rl">
        <img src={robotPoster} alt={translate({message: "MicroDuck 双足机器人仿真"})} width="88" height="72" />
        <div><span>{translate({message: "从这里开始一次机器人实验"})}</span><strong>{translate({message: "让 MicroDuck 学会行走"})}</strong><small>{translate({message: "MuJoCo 仿真 · 强化学习"})}</small></div>
        <ArrowUpRight size={19} aria-hidden="true" />
      </Link>
      <div className={styles.heroBottom}>
        <a href="#learning-paths" className={styles.scrollHint}>{translate({message: "向下探索学习路线"})}<ArrowDown size={14} aria-hidden="true" /></a>
        <div className={styles.sceneControls}>
          <span>{translate({message: "从数字仿真，到物理世界"})}</span>
          {motionAllowed && !failed && <button type="button" onClick={toggleVideo} aria-label={playing ? translate({message: "暂停背景动画"}) : translate({message: "播放背景动画"})} title={playing ? translate({message: "暂停背景动画"}) : translate({message: "播放背景动画"})}>{playing ? <Pause size={15} aria-hidden="true" /> : <Play size={15} aria-hidden="true" />}</button>}
        </div>
      </div>
    </header>
  );
    ...
    <section className={styles.playgrounds} aria-labelledby="playgrounds-title">
      <div className="home-container">
        <div className="home-section-heading">
          <div><p className="section-kicker">{translate({message: "交互实验"})}</p><h2 id="playgrounds-title">{translate({message: "动一下，原理就清楚了。"})}</h2></div>
          <p>{translate({message: "无需安装环境，在浏览器里试试控制与运动学。"})}</p>
        </div>
        <div className={styles.playgroundGrid}>
          {playgrounds.map(({number, title, subtitle, description, to, icon: Icon}) => (
            <Link to={to} className={styles.playgroundCard} key={to}>
              ...
              <span className="site-text-link">{translate({message: "打开实验"})}<ArrowUpRight size={17} aria-hidden="true" /></span>
            </Link>
          ))}
        </div>
      </div>
    ...
    <section className={styles.community} aria-labelledby="community-title">
      <div className="home-container">
        <div className={styles.communityInner}>
          <div><p className="section-kicker">{translate({message: "与 DATAWHALE 一起学习"})}</p><h2 id="community-title">{translate({message: "每一次复现，都让开源更进一步。"})}</h2><p>{translate({message: "分享实验结果，修正一处笔误，或提出一个好问题。"})}<br />{translate({message: "你的学习经验，也可以成为下一个人的起点。"})}</p></div>
          <div className={styles.communityLinks}>
            <a className="site-button site-button--primary" href="https://github.com/datawhalechina/dive-into-embodied-ai">{translate({message: "在 GitHub 参与共建"})}<ArrowUpRight size={18} aria-hidden="true" /></a>
            <div><a href="https://github.com/datawhalechina/dive-into-embodied-ai/issues">{translate({message: "反馈问题"})}</a><a href="https://github.com/datawhalechina/dive-into-embodied-ai/discussions">{translate({message: "参与讨论"})}</a></div>
          </div>
        </div>
      </div>
    </section>
