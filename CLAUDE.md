...
教程路径理念是"**认知 → 项目 → 面试**"三步走：先建立行业与技术栈整体认知，再在四类本体（机械臂、四足、人形、移动操作）上做能展示的项目，最终帮读者在面试中把项目经验讲清楚、拿到 offer。

写内容、做架构决策时优先问"**这能帮读者拿到 offer 吗**"，避免堆砌论文综述。占位页填充时先做能跑通的最小项目示例，再补理论。

## 三段入口骨架（2026-06 移除求职面试）

教程从旧的「总览 + 基础篇 + 实践篇 + 求职篇」重构为一级入口，并已在 2026-06 整体移除「求职面试」入口。顶部导航顺序：
...
> **重要约束**：用户对一级入口"什么进、什么出"非常明确，已转过几次结构。**2026-06 已整体移除「求职面试」一级入口**：删除 `docs/career/` 整个目录、`sidebars.ts` 里全部 `career*` 侧边栏、mega menu 的 `career` 项、首页第三张卡片与页脚链接。改动前先确认现状再下手，不要把已移除的入口又加回来。项目整体仍面向求职人群（首页 hero、`docs/overview`、`docs/foundations` 等处保留的「求职 / 面试」表述是定位描述，不是这个被删的入口）。
...
- Mega menu 数据（3 个一级入口下拉内容）：`src/components/NavbarMegaMenu/data.ts`
- Mega menu 样式：`src/components/NavbarMegaMenu/styles.css`
- 首页三张入口卡片：`src/components/HomepageFeatures/index.tsx`（用 `lucide-react` 图标，3 种 accent：cyan/green/blue；卡片用 `col col--4` 排成一行）
...
- 入口概述：`overviewSidebar` / `foundationsOverviewSidebar` / `practicesOverviewSidebar`
- 子专题：`foundations<Topic>Sidebar` / `practices<Topic>Sidebar`
- 内容多用 `autogenerated` 从目录生成；CS123 课程用显式 doc 列表锁定章节顺序

### 入口概述页

- 零基础入门：`docs/overview/intro.md`
- 理论技能树：`docs/foundations/intro.md`
