...
Real systems usually sit between the two: learned components replace some modules, or low-level controllers remain beneath a learned policy. The debate over monolithic and hierarchical designs in VLA research continues the same question in the foundation model period; see [From LLMs to robot policies](/docs/foundations/vla/vla_landscape).
